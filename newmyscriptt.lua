...
local function toggleFly1()
	...
	fly1Button.Text = fly1Enabled and "Stop FlyWalk [F]" or "FlyWalk [F]"
...
local function toggleFly2()
	...
	fly2Button.Text = fly2Enabled and "Stop Fly [R]" or "Fly [R]"
...
RunService.RenderStepped:Connect(function()
	if fly2Enabled and root and bodyVel then
		...
		local flatCFrame = CFrame.new(root.Position, root.Position + bodyVel.Velocity.Unit) * CFrame.Angles(math.rad(-90), 0, 0)
		root.CFrame = CFrame.new(root.Position) * CFrame.Angles(math.rad(-90), root.CFrame.YOrientation * math.rad(1), 0)
	end
end)
